...
class SMP_VK_Provider extends SMP_Provider {
	...
	public static function is_active() {
		return SMP_Options::get_option( 'setting_use_vkontakte' );
	}
	...
	public static function options() {
		return array(
			...
			'tab_caption'         => SMP_Options::get_option( 'setting_vkontakte_tab_caption' ),
			'css_class'           => 'vk-tab',
			'icon'                => 'fa-vk',
			'url'                 => esc_attr( SMP_Options::get_option( 'setting_vkontakte_page_url' ) ),
		);
	}
	...
	public static function container() {
		...
		$application_id = esc_attr( SMP_Options::get_option( 'setting_vkontakte_application_id' ) );
		if ( empty( $application_id ) ) {
			$application_id = 1;
		}

		$page_or_group_id = esc_attr( SMP_Options::get_option( 'setting_vkontakte_page_or_group_id' ) );
		...
		$content .= '<script type="text/javascript" src="//vk.com/js/api/openapi.js?105"></script>
			...
			<script type="text/javascript">
				...
				var smp_vk_container_height  = parseInt("' . absint( SMP_Options::get_option( 'setting_vkontakte_height' ) ) . '");
				...
				function initialize_VK_Widgets() {
					...
					jQuery.getScript( "//vk.com/js/api/openapi.js?115", function(data, textStatus, jqxhr) {
						...
						VK.Observer.subscribe("widgets.groups.joined", function f() {';
							...
							if ( SMP_Options::get_option( 'setting_vkontakte_close_window_after_join' ) ) {
								$content .= 'smp_destroyPlugin(smp_cookies.showWindowAfterReturningNDays);';
							}

							if ( self::$template->use_events_tracking() && SMP_Options::get_option( 'tracking_use_vkontakte' ) ) {
								$content .= self::$template->push_social_media_trigger_to_google_analytics( 'tracking_vkontakte_subscribe_event' );
						...
						VK.Observer.subscribe("widgets.groups.leaved", function f() {';
							...
							if ( self::$template->use_events_tracking() && SMP_Options::get_option( 'tracking_use_vkontakte' ) ) {
								$content .= self::$template->push_social_media_trigger_to_google_analytics( 'tracking_vkontakte_unsubscribe_event' );
						...
						VK.Widgets.Group("smp_vk_groups", {
							mode: '    . esc_attr( SMP_Options::get_option( 'setting_vkontakte_layout' ) ) . ',
							width: "'  . absint( SMP_Options::get_option( 'setting_vkontakte_width' ) ) . '",
							height: "' . absint( SMP_Options::get_option( 'setting_vkontakte_height' ) ) . '",
							color1: "' . esc_attr( SMP_Options::get_option( 'setting_vkontakte_color_background' ) ) . '",
							color2: "' . esc_attr( SMP_Options::get_option( 'setting_vkontakte_color_text' ) ) . '",
							color3: "' . esc_attr( SMP_Options::get_option( 'setting_vkontakte_color_button' ) ) . '"
						}, ' . $page_or_group_id . ');
